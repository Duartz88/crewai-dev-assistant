...
# Load .env BEFORE importing crewai/crewai-tools
...
from dev_assistant.crew import DevAssistantCrew  # noqa: E402
...
def _create_session_branch(project_path: str) -> str | None:
    ...
    if result.returncode == 0:
        ...
        console.print(f"[yellow]⚠️   Não foi possível criar branch:[/yellow] {result.stderr.strip()}")
        return None
    return branch
...
def _offer_fix_cycle(result, inputs: dict):
    ...
    DevAssistantCrew(project_path=inputs.get("project_path", "")).fix_crew().kickoff(inputs=fix_inputs)
...
def run():
    ...
    while True:
        ...
        try:
            ...
            DevAssistantCrew(project_path=project_path).implement_crew().kickoff(inputs=impl_inputs)

            console.print("\n[cyan]🔍  FASE 3/3 — Reviewer[/cyan]")
            review_result = DevAssistantCrew(project_path=project_path).review_crew().kickoff(inputs=impl_inputs)
...
def train():
    ...
    DevAssistantCrew(project_path=inputs["project_path"]).crew().train(
        n_iterations=int(sys.argv[1]), filename=sys.argv[2], inputs=inputs
    )


def replay():
    DevAssistantCrew().crew().replay(task_id=sys.argv[1])
...
def test():
    ...
    DevAssistantCrew(project_path=inputs["project_path"]).crew().test(
